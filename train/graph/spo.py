...
def train2S(dataloader, dataloader_val, optmodels, data_params, train_params):
    """
...
def trainSeparated(dataloader, dataloader_val, optmodels, data_params, train_params):
    ...
    for ind, (task, optmodel) in enumerate(optmodels.items()):
        ...
        # init SPO+ loss
        spop = pyepo.func.SPOPlus(optmodel, processes=proc)
        # set stopper
        stopper = earlyStopper(patience=5)
        # train mode
        reg[task].train()
        ...
        for epoch in tbar:
            ...
            for data in dataloader:
                ...
                # forward pass
                cp = reg[task](x)
                loss = spop(cp, c, w[:,ind], z[:,ind]).mean()
                # backward pass
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def trainSeparatedMSE(dataloader, dataloader_val, optmodels, data_params, train_params):
    """
    Train with separated model
    """
    m = data_params["node"] # number of nodes
    ...
    for ind, (task, optmodel) in enumerate(optmodels.items()):
        ...
        for epoch in tbar:
            ...
            for data in dataloader:
                ...
                # forward pass
                cp = reg[task](x)
                loss = spop(cp, c, w[:,ind], z[:,ind]).mean() + l2(cp, c)
                # backward pass
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
